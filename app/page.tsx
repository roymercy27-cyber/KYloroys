...
const SchoolIcon = () => (
  <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
    <path d="M14 22v-4a2 2 0 1 0-4 0v4"/><path d="M18 10V4a2 2 0 0 0-2-2H8a2 2 0 0 0-2 2v6"/><path d="M18 10a2 2 0 1 1 0 4h-1v2a2 2 0 1 1 0 4H7a2 2 0 1 1 0-4V14a2 2 0 1 1 0-4H7Z"/><path d="M11 20H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v1"/><path d="M18 16h3"/><path d="m14 16-3 3 3 3"/><path d="M11 19h7a2 2 0 0 0 2-2v-4a2 2 0 0 0-2-2h-7L11 8H4a2 2 0 0 0-2 2v10a2 2 0 0 0 2 2h7Z"/>
  </svg>
);
...
const PhoneIcon = () => (
  <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
    <path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 1.11L8.09 9.91a16 16 0 0 0 6 6l2.25-1.51a2 2 0 0 1 1.11-.45 12.84 12.84 0 0 0 2.81.72 2 2 0 0 1 1.72 2z"/>
  </svg>
);
...
export default function DashboardPage() {
  ...
  const [activeView, setActiveView] = useState<'dashboard' | 'outboundCall'>('dashboard');
    ...
    <div className="flex min-h-screen bg-slate-50 font-sans text-slate-900">
      ...
      <aside className="fixed inset-y-0 left-0 w-64 bg-orange-500 text-white p-6 flex flex-col gap-6 shadow-2xl z-50">
        <div className="flex flex-col items-center mb-4">
          <div className="bg-white rounded-2xl p-3 shadow-lg w-full flex flex-col items-center">
            <img 
              src="/logo.png" 
              alt="Kylo Roys Logo" 
            ...
            />
            <div className="text-center text-[10px] text-orange-600 font-bold uppercase tracking-tighter">
              School AI Receptionist
            </div>
          </div>
        </div>
        ...
        <nav className="flex flex-col gap-2">
          {['Dashboard', 'Calls', 'Contacts', 'Settings'].map((tab) => (
            <div 
              key={tab}
              onClick={() => { setActiveTab(tab); setView('main'); setActiveView('dashboard'); }}
        ...
        {activeTab === 'Dashboard' && (
          <div className="space-y-10">
            <div className="grid grid-cols-2 gap-6">
              <button 
                onClick={() => { setView('main'); setActiveView('dashboard'); }}
                className={`flex items-center justify-center gap-3 h-20 rounded-2xl font-bold text-lg transition-all shadow-sm ${activeView === 'dashboard' ? 'bg-orange-50 border-2 border-orange-500 text-orange-600' : 'bg-white border-2 border-slate-200 text-slate-400'}`}
              >
                <PhoneIcon /> Log Inbound Call
              </button>
              <button 
                onClick={() => { setView('outbound_call'); setActiveView('outboundCall'); }}
                className={`flex items-center justify-center gap-3 h-20 rounded-2xl font-bold text-lg transition-all shadow-lg ${activeView === 'outboundCall' ? 'bg-orange-600 text-white shadow-orange-200' : 'bg-orange-500 text-white opacity-80'}`}
              >
                Start Outbound Call →
              </button>
            </div>
            ...
            {activeView === 'dashboard' ? (
              ...
              <div className="p-24 flex flex-col items-center justify-center border-2 border-dashed border-slate-200 rounded-3xl text-center bg-white space-y-4">
                <div className="bg-slate-100 p-6 rounded-full text-slate-400">
                  <PhoneIcon />
                </div>
                <div className="text-3xl font-black text-slate-800">0 Outbound Calls</div>
                <div className="text-sm text-slate-500 max-w-sm">
                  Vapi Outbound triggers have not been configured for this environment yet.
                </div>
                <button 
